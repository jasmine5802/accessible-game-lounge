...
assert(main.includes('window.startLoungeGameFromPrompt?.') && games.includes('window.startLoungeGameFromPrompt=key=>') && games.includes("startStage!=='ready'") && games.includes('startGameFromPrompt();return true'), 'Electron Enter must directly invoke the game-ready start function before relying on asynchronous IPC fallback.');
