...
function beginSkipBo(room){const turnOrder=[...room.players.keys()],deck=SkipBoEngine.shuffle(SkipBoEngine.createDeck()),players=new Map(turnOrder.map(id=>[id,{name:room.players.get(id).name,stock:[],hand:[],discards:[[],[],[],[]]}])),stockSize=room.skipboPace==='Quick game'?10:SkipBoEngine.STOCK_SIZE;for(let round=0;round<stockSize;round+=1)for(const id of turnOrder)players.get(id).stock.push(deck.pop());const game={status:'playing',turnOrder,turnIndex:0,players,buildingPiles:[[],[],[],[]],drawPile:deck,completed:[],winnerId:null,sequence:1,announcement:`${room.skipboPace} Skip-Bo has started. ${players.get(turnOrder[0]).name} goes first.`};refillSkipBo(game,players.get(turnOrder[0]));room.skipbo=game}
