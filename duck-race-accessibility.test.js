...
assert(horseHtml.includes('id="target" class="hand" role="listbox"') && horseRace.includes('function renderTargets()') && horseRace.includes('selectedTargetIndex'), 'Horse Race must show each other player as a named, keyboard-selectable target.');
...
assert((server.match(/name: 'Trivia —/g) || []).length >= 20 && ['Animals','Science','Geography','History','Math','Language','Books','Music','Art','Sports','Food','Technology','Nature','Space','Everyday Life'].every(category=>server.includes(`Trivia — ${category}`)), 'Duck Race and Horse Race must share broad, rotating trivia categories.');
...
assert(server.includes('Lap ${nextLap} of ${DUCK_RACE_LAPS} begins now.'), 'Duck Race must announce the number of the next lap when a duck crosses the line.');
...
assert(['Clover Boost','Loose Horseshoe','Oat Snack','Soft Mud'].every(card=>horseEngine.includes(card)&&server.includes(card)) && server.includes('card.selfPlacement'), 'Horse Race must include action cards dropped on the current horse space.');
assert(['Lasso','Headwind','Position Swap','Mud Sling','Hay Bale','Speed Gate','Trip Wire'].every(card=>horseEngine.includes(card)) && horseRace.includes('targetOpponents()') && server.includes("if(card.target)"), 'Horse Race must retain named-opponent targeting cards like Duck Race.');
assert(horseRace.includes("['arrowup','arrowdown'].includes(key)&&game?.status==='playing'&&!game.pendingMiniGame") && horseRace.indexOf("Card ${selected+1} of ${game.myHand.length}. Press Enter to play.") < horseRace.lastIndexOf('accessibility?.handleKey(event)'), 'Horse Race Up and Down must select and speak cards from anywhere on the game screen, like Duck Race.');
assert(horseRace.includes("${fresh?`${game.announcement} `:''}It is your turn"), 'Horse Race must announce the previous player’s action before announcing a new turn.');
assert(horseRace.includes('window.LoungeAccessibility?.accessibleMode') && horseRace.includes('window.LoungeAccessibility.speak?.(message)'), 'Horse Race accessible mode must speak every card reached with Up or Down instead of relying only on a live region.');
...
assert(['Sound Match','Memory Challenge','Sequence Challenge','Odd One Out','Word Puzzle','Quick Count'].every(gameName=>server.includes(`name: '${gameName}'`)) && server.includes('room.raceMiniGameDeck.pop()'), 'Race mini-games must include non-trivia challenges and avoid repeats until the full pool is exhausted.');
assert(server.includes('racer.hand.push(randomCard())') && server.includes('if(racer)racer.hand.push(dealDerbyCard(game))'), 'Every correct race mini-game answer must award an Action Card.');
assert(['Jump Rail','Rutted Track','Apple Basket','Dust Cloud','Card Swipe','Gate Lock','Golden Rail'].every(card=>horseEngine.includes(card)&&server.includes(card)), 'Horse Race must include additional self-drop and opponent-target cards with server effects.');
...
assert(source.includes('miniAnswerIndex = (miniAnswerIndex + direction + buttons.length) % buttons.length') && horseRace.includes('miniAnswerIndex=(miniAnswerIndex+direction+buttons.length)%buttons.length'), 'Trivia answers must keep an explicit selection so every arrow press reaches the next answer even if browser focus is reset.');
assert(source.includes('Choices: ${answerChoices}') && horseRace.includes('Choices: ${answerChoices}'), 'Trivia prompts must announce every available answer choice when a mini-game opens.');
...
assert(audioSource.includes('window.playWinnerFanfare') && source.includes('playWinnerFanfare') && horseRace.includes('playWinnerFanfare'), 'Both race games must play a distinct winner fanfare with their animal sound.');
