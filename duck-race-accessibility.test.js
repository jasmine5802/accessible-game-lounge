...
assert(horseRace.includes('DerbyEngine.CARDS[pendingCard]?.placement') && horseRace.includes('targetSquare:Number(el.targetSquare.value)'), 'Horse Race placement cards must expose an exact lane-square choice.');
...
assert(server.includes('requested=Number(data.targetSquare)') && server.includes('target.mudHazards.add(square)'), 'The server must honor a selected Horse Race Mud Sling square.');
