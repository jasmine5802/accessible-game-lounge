...
function selectedButton(menu,index){ return menu.querySelectorAll('button')[index]; }
function setSelection(menu,index,voice=true){ menu.querySelectorAll('button').forEach((button,i)=>{button.setAttribute('aria-selected',String(i===index));button.tabIndex=i===index?0:-1;}); const button=selectedButton(menu,index); button?.focus({preventScroll:false}); if(voice&&button)announce(button.textContent); }

function renderGames(){
  elements.gameMenu.replaceChildren(...GAME_TITLES.map(title=>{const li=document.createElement('li'),button=document.createElement('button');button.type='button';button.setAttribute('role','option');button.textContent=title;button.addEventListener('click',()=>chooseGame(title));li.append(button);return li;}));
  setSelection(elements.gameMenu,gameIndex,false);
}
function renderTables(){
  const options=[{create:true,label:'Create a new table'},...tables.map(table=>({table,label:`Join ${table.host}'s ${selectedGame} table [${table.playerCount}/${table.maxPlayers} players]`}))];
  elements.tableMenu.replaceChildren(...options.map(option=>{const li=document.createElement('li'),button=document.createElement('button');button.type='button';button.setAttribute('role','option');button.textContent=option.label;button.addEventListener('click',()=>option.create?createTable():joinTable(option.table.id));li.append(button);return li;}));
  tableIndex=Math.min(tableIndex,options.length-1); setSelection(elements.tableMenu,tableIndex,false);
}
function chooseGame(title){ selectedGame=title; swipeSound(); socket.emit('get-game-tables',{category:GAME_CATEGORIES[title]},result=>{if(!result.ok)return announce(result.error);tables=result.tables;tableIndex=0;elements.lobby.hidden=true;elements.tablePicker.hidden=false;elements.tablesTitle.textContent=`${title} Tables`;screen='tables';renderTables();announce(`${title}. ${tables.length} open table${tables.length===1?'':'s'}. Create a new table selected.`);}); }
...
function showWaiting(room){ currentRoom=room;sessionStorage.setItem('loungeGameId',room.id);elements.lobby.hidden=true;elements.tablePicker.hidden=true;elements.waitingTable.hidden=false;screen='waiting';updateWaiting(room); }
...
function enterLobby(result){ myUsername=result.username;sessionStorage.setItem('loungeUsername',myUsername);if(result.token)sessionStorage.setItem('loungeSessionToken',result.token);elements.password.value='';elements.authScreen.hidden=true;elements.lobby.hidden=false;screen='games';renderGames();setSelection(elements.gameMenu,gameIndex,false);announce(`Welcome, ${myUsername}. Monopoly selected. Use Up and Down Arrow, then Enter.`); }
